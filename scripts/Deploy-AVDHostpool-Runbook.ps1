param(
    [Parameter(Mandatory=$true, HelpMessage='The name of the parameter file located in Azure Storage to deploy')]
...
$dcJoinSecretEntry = "avdhostpool-dcJoinSecret"
...
foreach($pool in $deploymentConfig)
{
    ...
    ## $ResourceGroupName = $pool.ResourceGroupName.value
    $resourceGroupTags = $pool.hostPoolTags.value
    ...
    try {
        ...
        ## $deploymentParamsHash = @{
        ##     "tokenExpirationTime" = $tokenExpirationTime
        ##     "administratorAccountPassword" = "$($dcJoinpasswordSec)"
        ##     "vmAdministratorAccountPassword" = "$($vmAdminSecretSec)"
        ## }
        ## $templateDeploymentParams = Get-AzTemplateParameters -ParametersFilePath $parameterFile -TemplateParameterObject $deploymentParamsHash
        ## ORIG: Test-AzResourceGroupDeployment -ResourceGroupName $ResourceGroupName -TemplateFile "$deploymentTemplate" -TemplateParameterObject $templateDeploymentParams -Verbose
    ...
    try {
        ...
        ## }
        ## $templateDeploymentParams = Get-AzTemplateParameters -ParametersFilePath $parameterFile -TemplateParameterObject $deploymentParamsHash
        ## New-AzResourceGroupDeployment -ResourceGroupName $ResourceGroupName -TemplateFile "$deploymentTemplate" -TemplateParameterObject $templateDeploymentParams -Verbose
